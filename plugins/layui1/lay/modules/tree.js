    ...
    i.prototype.expand = function (treeNode, isOpened, e) {
        ...
        i.prototype.treeGird = function (e, a) {
            ...
            layui.each(n, function (a, n) {
                ...
                var str = o(['<tr class="' + (p ? "layui-hide" : "") + '" id="' + n.id + '">',
                        function () {
                            if (i.checkbox) {
                        ...
                        function () {
                            ...
                            for (var ind = 0; ind < i.layout.length; ind++) {
                                if (i.layout[ind].treeNodes) {
                                    ret += '<td class="' + i.layout[ind].colClass + '" style="' + i.layout[ind].style + '"><li ' + (n.spread ? 'data-spread="' + n.spread + '"' : "") + '>' + (indent + (l ? '<i class="layui-icon layui-tree-spread">' + (n.spread ? t.arrow[1] : t.arrow[0]) + "</i>" : "")) + '<a href="' + (n.href || "javascript:;") + '" ' + (i.target && n.href ? 'target="' + i.target + '"' : "") + ">" + ('<i class="fa fa-' + (l ? n.spread ? "folder-o" : "folder" : "id-card-o") + '">' + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a></li></td>");
        ...
        i.prototype.treeGirdLoad = function (e, a ,nt) {
            var r = this,
                i = r.options,
                n = a || i.nodes;
            layui.each(n, function (a, n) {
                ...
                e.after(str), index++, l && (r.treeGirdLoad(e, n.children)), r.spreadGird(str, n, e.selector), i.drag && r.drag(str, n)
                r.changed(str, n)
            })
        },
        ...
        i.prototype.spreadGird = function (e, o, el) {
            var a = this,
                i = a.options,
                r = (i, e.find(".layui-tree-spread")),
                nodeId = e[0].id,
                ri = e.find(".fa"),
                nt = tt[el],
                l = function () {
                    a.children(e,nt,nodeId)
                };
            (r.on("click", l), r.on("dblclick", l))
        },
        i.prototype.children=function (e,nt,nodeId) {
            var a = this,
                i = a.options,
                b=i,
                r = (i, e.find(".layui-tree-spread")),
            ...
            var parentNode=nt.mapping[treeNode.id];
            a.expand(treeNode, !isOpened, e);
            isOpened ? (e.data("spread", null), r.html(t.arrow[0]), ri.addClass("fa-folder").removeClass("fa-folder-o")) : (e.data("spread", !0), r.html(t.arrow[1]), ri.addClass("fa-folder-o").removeClass("fa-folder"))
            treeNode.isOpened = !isOpened;
            if(treeNode.isOpened&&i.type==="load"&&!parentNode.isLoad){
                i.loadData.id=treeNode.id;
                biyue.ajax({
                    url:i.loadUrl,
                    data:i.loadData,
                    fun:function (data) {
                        parentNode.isLoad=true;
                        var dataV = data.value,dataO=[];
                        dataV.reverse();
                        for(var i=0;i<dataV.length;i++){
                            ...
                            nt.mapping[node.id] = node;
                        }
                        a.treeGirdLoad(e,dataV,nt);
                        layui.form.render();
                        //加载完后执行事件
                        if(b.methods){
                            b.methods.openOk&& b.methods.openOk(tt[b.elem].mapping);
                        }
                    }
                });
            }
        ...
        e("treeGird", function (e) {
            ...
            var v = r.initGird(t);
            if(e.methods){
                e.methods.del&&e.methods.del(tt[e.elem].mapping);
            }
